...
// GET /api/applications/buyer/admin - Get all buyer applications (admin only)
export async function GET(request) {
  try {
    ...
    if (error) {
      console.error('Get buyer applications error:', error)
...
// PATCH /api/applications/buyer/admin - Update buyer application status (admin only)
export async function PATCH(request) {
  try {
    ...
    if (create_user && status === 'approved') {
      ...
      if (userError) {
        console.error('Create buyer user error:', userError)
        return Response.json({ 
      ...
      await supabaseAdmin
        ...
        .insert({
          ...
          action: 'buyer_user_created_from_application',
      ...
      // TODO: Send email with credentials to the new buyer
      console.log('Buyer user created from application:', {
        user_id: newUser.id,
        email: newUser.email,
    ...
    if (updateError) {
      console.error('Update buyer application error:', updateError)
      return Response.json({ 
    ...
    // Log audit
    await supabaseAdmin
      .from('buyer_application_audit_log')
      .insert({
        admin_id: auth.user.id,
    ...
    if (updateData.temp_password) {
      ...
      response.message = 'Application approved and buyer account created successfully'
    }

    return Response.json(response)

  } catch (error) {
    console.error('Update buyer application error:', error)
    return Response.json({ error: 'Internal server error' }, { status: 500 })
  }
}

// DELETE /api/applications/buyer/admin - Delete buyer application (admin only)
export async function DELETE(request) {
  try {
    ...
    if (deleteError) {
      console.error('Delete buyer application error:', deleteError)
      return Response.json({ 
    ...
    console.error('Delete buyer application error:', error)
    return Response.json({ error: 'Internal server error' }, { status: 500 })
  }
}